...
admin-client-section-logo = Логотип на экране согласия
admin-client-logo-intro = Показывается пользователям на экране согласия для этого приложения. Загружается здесь, а не берётся из logo_uri клиента, поэтому изображение отдаётся с этого сервера и IP-адрес пользователя не попадает в приложение до того, как он даст согласие.
admin-client-logo-file = Файл логотипа
admin-client-logo-hint = PNG, JPEG или WebP, до 256 КБ. Лучше всего подходят квадратные изображения.
admin-client-logo-remove = Удалить текущий логотип
admin-client-logo-save = Сохранить логотип
...
admin-status-audit-webhook-accept-list = Записи списка разрешённых токенов аудит-вебхука
admin-status-audit-webhook-last-matched = Последняя сработавшая запись аудит-вебхука
admin-status-audit-webhook-last-matched-none = нет с момента запуска
