...
admin-client-section-logo = Consent screen logo
admin-client-logo-intro = Shown to users on the consent screen for this app. Uploaded here rather than taken from the client's own logo_uri, so the image is served from this server and no user's IP reaches the app before they consent.
admin-client-logo-file = Logo file
admin-client-logo-hint = PNG, JPEG or WebP, up to 256 KB. Square images work best.
admin-client-logo-remove = Remove the current logo
admin-client-logo-save = Save logo
