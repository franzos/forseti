...
admin-client-section-logo = Logo de l'écran de consentement
admin-client-logo-intro = Affiché aux utilisateurs sur l'écran de consentement de cette application. Téléversé ici plutôt que repris du logo_uri du client, afin que l'image soit servie par ce serveur et qu'aucune adresse IP d'utilisateur n'atteigne l'application avant son consentement.
admin-client-logo-file = Fichier du logo
admin-client-logo-hint = PNG, JPEG ou WebP, jusqu'à 256 Ko. Les images carrées conviennent le mieux.
admin-client-logo-remove = Supprimer le logo actuel
admin-client-logo-save = Enregistrer le logo
...
admin-status-audit-webhook-accept-list = Entrées de la liste d'acceptation du webhook d'audit
admin-status-audit-webhook-last-matched = Dernière entrée correspondante du webhook d'audit
admin-status-audit-webhook-last-matched-none = aucune depuis le démarrage
