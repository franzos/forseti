...
admin-client-section-logo = Logo für den Zustimmungsdialog
admin-client-logo-intro = Wird Nutzern im Zustimmungsdialog dieser App angezeigt. Hier hochgeladen statt aus der logo_uri des Clients übernommen, damit das Bild von diesem Server ausgeliefert wird und keine IP-Adresse eines Nutzers die App erreicht, bevor er zustimmt.
admin-client-logo-file = Logodatei
admin-client-logo-hint = PNG, JPEG oder WebP, bis zu 256 KB. Quadratische Bilder funktionieren am besten.
admin-client-logo-remove = Aktuelles Logo entfernen
admin-client-logo-save = Logo speichern
...
admin-status-audit-webhook-accept-list = Einträge der Audit-Webhook-Accept-List
admin-status-audit-webhook-last-matched = Zuletzt getroffener Audit-Webhook-Eintrag
admin-status-audit-webhook-last-matched-none = keiner seit Start
