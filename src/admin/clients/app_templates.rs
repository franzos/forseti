...
//! `oauth_client_metadata.template_slug`, purely cosmetically — it drives
//! the app logo on the client list. It is NOT trust state; nothing reads
...
pub(super) struct AppTemplate {
    ...
    /// generated by Hydra, so this happens via a follow-up update.
    ...
    /// Logo filename in static/logos/, e.g. Some("gitlab.svg"). None → letter-tile fallback.
    pub(super) logo: Option<&'static str>,
    /// Light variant filename used in the dark theme; None → use `logo` in both themes.
...
pub(crate) struct AppCard {
    ...
    /// Logo filename in static/logos/; None → letter-tile fallback.
    pub(crate) logo: Option<&'static str>,
    /// Light-theme logo variant; None → use `logo` in both themes.
...
mod tests {
    ...
    fn liwan_has_a_dark_variant() {
        // Liwan's mark is monochrome black, so it needs a light variant to
