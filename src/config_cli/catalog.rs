...
pub(crate) const SETTINGS: &[Setting] = &[
    ...
    Setting {
        ...
        description: "Registers Forseti as an upstream OIDC client of GitHub. Create an OAuth App at github.com/settings/developers, set the callback URL to `https://<kratos-public-host>/self-service/methods/oidc/callback/github`, request the `user:email` scope so an email is returned at all, then paste the client ID and secret into kratos.yml. GitHub isn't an OIDC provider here: Kratos reads the account's primary address and its verified flag from the REST API. Forseti's pinned mapper gates the `email` trait on that flag but does not carry the verification into Kratos — GitHub users verify through Forseti's own flow before verified-only features open up.",
        targets: &["kratos.yml"],
    },
    Setting {
        key: "oidc.microsoft",
        ...
        description: "Registers Forseti as an upstream OIDC client of Microsoft Entra ID (Azure AD). Register an app at the Entra admin center, set the redirect URI to `https://<kratos-public-host>/self-service/methods/oidc/callback/microsoft`, then paste the client ID and secret into kratos.yml. `microsoft_tenant` must name a specific tenant ID: `common`, `organizations`, and `consumers` are all refused, since each accepts sign-in from tenants you don't control and Entra's `email` is a mutable directory attribute any tenant admin can point at any address (the nOAuth account-takeover class). Entra issues no `email_verified` claim at all, so Forseti never carries Microsoft verification into Kratos — those users register an email through Forseti and verify it there.",
        targets: &["kratos.yml"],
    },
    Setting {
        key: "oidc.apple",
